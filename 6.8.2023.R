...
# Import des Datensatz fuer den Einkommens-Gini-Koeffizienten (online unter: https://data.worldbank.org/indicator/SI.POV.GINI?end=2022&start=2022&view=bar)
wb_income_gini <- read_csv("Worldbankd_IncomeGINI_Data_v2022.csv", skip = 4) %>%
  pivot_longer(cols=5:67,
               values_to="income_gini",
               names_to="year") %>%
  select(year,
         state_name = "Country Name",
         income_gini) %>%
...
# Auswaehlen des letzten nicht NAs-Wert als aktuellen Wert fuer den Einkommens-Gini im Jahr 2022
# Erstellen einer Variablle "year_income_gini", die angibt aus welchem Jahr der letzte aktuelle Wert fuer den Einkommens-Gini stammt.
wb_income_gini_2022 <- wb_income_gini %>%
  filter(!is.na(income_gini)) %>%
  group_by(state_name) %>%
  summarise(year_income_gini = year,
            income_gini = case_when(year == max(year) ~ income_gini,
                                    TRUE ~ NA)
            ) %>%
  mutate(year = 2022) %>%
  filter(!is.na(income_gini)) %>%
...
# Vereinigung des Datensatz fuer den Einkommens-Gini-Koeffizienten mit dem Datensatz fuer demokratische und autokratische Regime:
vdem <- vdem %>%
  left_join(wb_income_gini_2022,
            by=c("year",
                 "state_name")
  )
...
# Visualisierung des BIPs von China, Daenemark, Finland, Frankreich, Deutschland, das Vereinigte Kaenigreich und Schweden:
vdem %>% filter(year == 2022 &
                  state_name %in% c("China","Denmark","Finland","France","Germany","United States",
  ...
  labs(title = "The lie of wealth through oppression III",
       subtitle = "GDP per capita in major western democracies ... and Chima (2022)",
       y = "GDP per Capita (1000$)",
       caption = "Source: Worldbank") +
  theme_kupfer1 +
  theme(legend.position = "none")

# Visualisierung der Entwicklung des BIPs von China, Daenemark, Finland und Schweden:
vdem %>%
  filter(state_name %in% c("China","Denmark","Finland","Sweden")) %>%
  ggplot( aes(x = year, y = gdp_per_capita, color = state_name) ) +
  geom_line() +
  labs(title = "The lie of wealth through oppression IV",
       subtitle = "development of GDP per capita for nordic countries (1960-2022)",
       color = "Country:",
       y = "GDP per capita (1000$)",
       caption = "Source: Worldbank") +
  theme_kupfer1
...
# Visualisierung des Einkommens-Ginis nach Regimetyp:
vdem %>%
  mutate(type_of_regime = factor(type_of_regime,
                                 levels = c(0,1,2,3),
                                 labels = c("closed autocracy",
  ...
  filter(year == 2022) %>%
  ggplot( aes(x = type_of_regime, y = income_gini, fill = type_of_regime) ) +
  geom_boxplot() +
  labs(title = "The lie of solidarity through oppression I",
       subtitle = "GINI-coefficient for income inequality per regime type (2022)",
       y = "GINI-coefficient for income inequality (0-1)*",
       caption = "*0 = perfect equality
       100 = perfect inequality
       Source: V-dem Institute; Worldbank") + 
  theme_kupfer1 +
  theme(legend.position = "none")

# Visualisierung des Einkommens-GINI von China, Daenemark, Finland, Frankreich, Deutschland, das Vereinigte Kaenigreich und Schweden:
vdem %>% filter(year == 2022 &
                  state_name %in% c("China","Denmark","Finland","France","Germany","United States",
                                    "United Kingdom","Sweden")
) %>%
  mutate() %>%
  ggplot( aes(x = reorder(state_name,desc(income_gini)), y = income_gini) ) +
  geom_bar(stat = "identity",
           fill = brewer.pal(9,"Set1")[2],
           width = 0.75) +
  labs(title = "The lie of solidarity through oppression II",
       subtitle = "GINI-coefficient for income inequality in major western democracies ... and China (2022)",
       y = "GINI-coefficient for income inequality (0-1)",
       caption = "*0 = perfect equality
       100 = perfect inequality
       Source: Worldbank") +
